...
export default function ChapterModal() {
  ...
  const progress = selectedChapter.lessonsCount
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1.5">
              <span id="modal-chapter-count" className="text-xs font-bold text-slate-500 dark:text-slate-400">
                {selectedChapter.completedLessons} of {selectedChapter.lessonsCount} Lessons Complete
              </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 flex-1 overflow-y-auto min-h-0">
          ...
          <div id="modal-lessons-list" className="md:col-span-2 p-6 space-y-4 overflow-y-auto">
            {selectedChapter.lessons.map((lesson) => (
              <div key={lesson.id} className="bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-full bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400 flex items-center justify-center flex-shrink-0 mt-0.5">
                    {lesson.completed ? <Check className="w-5 h-5 stroke-2" /> : <Play className="w-4 h-4 fill-current ml-0.5" />}
                  </div>
                  <div>
                ...
                <button
                  type="button"
                  onClick={() => launchLesson(lesson.type, lesson.title)}
                  className="px-5 py-2 rounded-xl border-2 border-purple-600 text-purple-600 dark:text-purple-400 hover:bg-purple-600 hover:text-white dark:hover:text-white text-xs font-bold transition-all whitespace-nowrap self-end sm:self-center"
                >
