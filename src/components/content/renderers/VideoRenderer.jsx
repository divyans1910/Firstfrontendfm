...
import { resolveVideoEmbed } from '../../../lib/videoEmbed.js';

export default function VideoRenderer({ manifest }) {
  const data = manifest.data || {};
  const embed = resolveVideoEmbed(data.url);
    ...
    <article className="bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-slate-200/80 dark:border-slate-700/80 space-y-4">
      ...
      <div className="relative aspect-video rounded-2xl overflow-hidden bg-slate-900 border border-slate-800">
        {embed?.kind === 'video' ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-sm text-white/80">No video URL provided</div>
        )}
      </div>
    </article>
  );
